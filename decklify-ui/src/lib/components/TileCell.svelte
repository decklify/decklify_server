...
{#if tile}
  ...
  bg-white/10 rounded-xl flex flex-col items-center justify-center
    w-full h-full border-2 cursor-grab active:cursor-grabbing
    ...
    {_state.dragOverIndex === index && _state.dragSourceIndex !== index
      ...
      : _state.selectedTileIndex === index
        ...
        : 'border-white/10 hover:border-blue-500 hover:-translate-y-1 hover:bg-white/10'}
  ...
  >
    ...
    {/if}
    {#if tile.label}
      <span class="text-2xl font-bold mt-2 text-white/85">{tile.label}</span>
    {/if}
  </button>
{:else}
  <div
